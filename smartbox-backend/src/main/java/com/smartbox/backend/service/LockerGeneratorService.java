...
public class LockerGeneratorService {
    ...
    public List<Locker> generate(double userLat, double userLng) {
        ...
        while (lockers.size() < LOCKER_COUNT && attempts < maxAttempts) {
            ...
            String prefix = lockerNames[lockers.size()];
            for (int i = 1; i <= 5; i++) {
                slots.add(prefix + i);
            }
            ...
            locker.setAvailableSlots(1 + random.nextInt(5));
            lockers.add(locker);
        }
    ...
    private double[] generateRandomPoint(double userLat, double userLng) {
        double distance = Math.sqrt(random.nextDouble()) * MAX_RADIUS_METERS;
        double angle = random.nextDouble() * 2 * Math.PI;

        double latRad = Math.toRadians(userLat);
        double lngRad = Math.toRadians(userLng);
    ...
    private boolean tooClose(double lat, double lng, List<Locker> lockers) {
        for (Locker locker : lockers) {
            double distance = calculateDistance(lat, lng, locker.getLatitude(), locker.getLongitude());
